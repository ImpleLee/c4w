...
impl Minimizer for ParallelMinimizer {
  fn minimize<T: States+std::marker::Sync+HasLength>(states: T) -> MappedStates<T> {
    ...
    loop {
      let global = seeds
        .par_iter()
        .map(|&state| (states.get_next_id(state, &*state2num), state))
        .collect::<HashMap<_, _>>();
      ...
      let local_seeds = new_state2num
        ...
        .fold(HashMap::new, |mut local, (i, ret)| {
          let next = states.get_next_id(i, &*state2num);
    ...
    let seed_map =
      seeds.par_iter().enumerate().map(|(i, &seed)| (seed, i)).collect::<HashMap<_, _>>();
    state2num.par_iter_mut().for_each(|res| {
      *res = seed_map[res];
    });
