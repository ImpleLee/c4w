...
impl<T: States + std::marker::Sync + HasLength> Minimizer<T> for RawMinimizer {
  fn minimize(states: T) -> MinimizedStates<T> {
    ...
    loop {
      let next_set = (0..res.len()).into_par_iter().map(|i| states.get_next(i, &res)).collect::<HashSet<_>>();
      ...
      let next_map = nexts.par_iter().enumerate().map(|(i, next)| (next, i)).collect::<HashMap<_, _>>();
      res = (0..res.len()).into_par_iter().map(|i| next_map[&states.get_next(i, &res)]).collect::<Vec<_>>();
    }
    MinimizedStates {
      states,
      state2num: res,
      nexts: nexts.into_iter().collect(),
    }
  }
}
